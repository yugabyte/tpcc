...
public class TPCCUtil {
     ...
     */
  public Customer newCustomerFromResults(ResultSet rs)
      throws SQLException {
  ...
  private final RandomGenerator ran = new RandomGenerator(0);

  public String randomStr(int strLen) {
      if (strLen > 1)
  ...
  public String randomNStr(int stringLength) {
    if (stringLength > 0)
  ...
  // these each time.
  private final int OL_I_ID_C = 7911; // in range [0, 8191]
  private final int C_ID_C = 259; // in range [0, 1023]
  // NOTE: TPC-C 2.1.6.1 specifies that abs(C_LAST_LOAD_C - C_LAST_RUN_C) must
  // be within [65, 119]
  private final int C_LAST_LOAD_C = 157; // in range [0, 255]
  private final int C_LAST_RUN_C = 223; // in range [0, 255]

  public int getItemID(Random r) {
    return nonUniformRandom(8191, OL_I_ID_C, 1, configItemCount, r);
  }

  public int getCustomerID(Random r) {
  ...
  public String getNonUniformRandomLastNameForRun(Random r) {
    return getLastName(nonUniformRandom(255, C_LAST_RUN_C, 0, 999, r));
  }

  public String getNonUniformRandomLastNameForLoad(Random r) {
   ...
   */
  public int getRandomWarehouseId(Worker w, int warehouseId, int numWarehouses, Random gen) {
      if (!w.getBenchmarkModule().getWorkloadConfiguration().getGeoPartitioningEnabled()) {
          return this.randomNumber(1, numWarehouses, gen);
      }
  ...
  public int randomNumber(int min, int max, Random r) {
    return (int) (r.nextDouble() * (max - min + 1) + min);
  }

  public int nonUniformRandom(int A, int C, int min, int max, Random r) {
