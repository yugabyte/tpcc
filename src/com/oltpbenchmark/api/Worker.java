        ...
        try {
            if(!wrkld.getUseHikariPool()) {
                ...
                if(wrkld.getUseCreateConnForEveryTx())
                    ...
                    LOG.info("Creating a persistent connection");
            ...
            while (status == TransactionStatus.RETRY &&
                    ...
                    if (ex.getSQLState() != null) {
                        if (ex.getErrorCode() == 0 && ex.getSQLState() != null && ex.getSQLState().equals("40001")) {
                            ...
                            if(attempt == totalAttemptsPerTransaction) {
                                LOG.error("The DBMS rejected the transaction without an error code and max retry attempts reached : " +  ex.getStackTrace());
                            } else {
                ...
                // Assertion Error
                    ...
                    if (attempt == totalAttemptsPerTransaction) {
                        ...
                        LOG.warn("Fatal error when invoking :" + ex.getMessage() + ". Retrying...");
                    }
                    status = TransactionStatus.RETRY;
                 // Random Error
                } catch (Exception ex) {
