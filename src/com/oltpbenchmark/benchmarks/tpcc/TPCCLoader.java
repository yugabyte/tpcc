...
public class TPCCLoader extends Loader<TPCCBenchmark> {
    ...
    protected void DisableForeignKeyConstraints(Connection conn) {
        try {
            Statement st = conn.createStatement();
            String sql;

            sql = "ALTER TABLE CUSTOMER DROP CONSTRAINT C_FKEY_D";
            st.execute(sql);
            sql = "ALTER TABLE OORDER DROP CONSTRAINT O_FKEY_C";
            st.execute(sql);
            sql = "ALTER TABLE NEW_ORDER DROP CONSTRAINT NO_FKEY_O";
            st.execute(sql);
            sql = "ALTER TABLE HISTORY DROP CONSTRAINT H_FKEY_C";
            st.execute(sql);
            sql = "ALTER TABLE HISTORY DROP CONSTRAINT H_FKEY_D";
            st.execute(sql);
            sql = "ALTER TABLE ORDER_LINE DROP CONSTRAINT OL_FKEY_O";
            st.execute(sql);
            sql = "ALTER TABLE ORDER_LINE DROP CONSTRAINT OL_FKEY_S";
            st.execute(sql);
