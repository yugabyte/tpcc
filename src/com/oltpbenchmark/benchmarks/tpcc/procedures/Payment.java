...
public class Payment extends Procedure {
  ...
  public static final InstrumentedSQLStmt payGetCustCdataSQL = new InstrumentedSQLStmt(
      "SELECT C_DATA " +
      "  FROM " + TPCCConstants.TABLENAME_CUSTOMER +
      " WHERE C_W_ID = ? " +
      "   AND C_D_ID = ? " +
  ...
  public static final InstrumentedSQLStmt payUpdateCustBalCdataSQL = new InstrumentedSQLStmt(
  ...
  public static final InstrumentedSQLStmt payUpdateCustBalSQL = new InstrumentedSQLStmt(
      "UPDATE " + TPCCConstants.TABLENAME_CUSTOMER +
      "   SET C_BALANCE = ?, " +
      "       C_YTD_PAYMENT = ?, " +
      "       C_PAYMENT_CNT = ? " +
  ...
  public static final InstrumentedSQLStmt payInsertHistSQL = new InstrumentedSQLStmt(
    ...
    if (c.c_credit.equals("BC")) { // bad credit
      ...
      if (result == 0)
        throw new RuntimeException("Error in PYMNT Txn updating Customer C_ID=" + c.c_id +
                                   " C_W_ID=" + customerWarehouseID + " C_D_ID=" +
                                   customerDistrictID);
    } else { // GoodCredit
  ...
  public Customer getCustomerByName(int c_w_id, int c_d_id,
    ...
    if (customers.size() == 0) {
      throw new RuntimeException("C_LAST=" + customerLastName + " C_D_ID=" + c_d_id + " C_W_ID=" +
                                 c_w_id + " not found!");
    }
